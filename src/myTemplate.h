...
template<typename T>
void add3(T z[3], const T x[3], const T y[3]);

template<typename T>
T dot3(const T x[3], const T y[3]);

template<typename T>
T dot3Long(const T *x, const T y[3], int n);

template<typename T>
void copy3(T z[3], const T x[3]);

template<typename T>
void add(T *z, const T *x, const T *y, int d);

template<typename T>
T dot(const T *x, const T *y, int d);
...
template<typename T>
void jacob(T z, const T x[2], const T y[2]);
...
template<typename T>
void crossDiff(T z[3], const T fu[3], const T fv[3], const T gu[3], const T gv[3]);

template<typename T>
void crossLong(T z[3], const T *fu, const T *fv, const T *gu, const T *gv, int n);
...
template<typename T>
inline void add3(T z[3], const T x[3], const T y[3]) {
	z[0] = x[0] + y[0];
    z[1] = x[1] + y[1];
    z[2] = x[2] + y[2];
}
...
template<typename T>
inline T dot3(const T x[3], const T y[3]) {
    T s = 0;
...
template<typename T>
inline T dot3Long(const T *x, const T y[3], int n) {
    T s = 0;
    s = x[0*n]*y[0] + x[1*n]*y[1] + x[2*n]*y[2];
    return s;
}

template<typename T>
inline void copy3(T z[3], const T x[3]) {
	z[0] = x[0];
    z[1] = x[1];
    z[2] = x[2];
}
...
template<typename T>
inline void crossDiff(T z[3], const T fu[3], const T fv[3], const T gu[3], const T gv[3]) {
    z[0] = fu[1]*gv[2]-fu[2]*gv[1] + gu[1]*fv[2]-gu[2]*fv[1];
	z[1] = -fu[0]*gv[2]+fu[2]*gv[0] -gu[0]*fv[2]+gu[2]*fv[0];
	z[2] = fu[0]*gv[1]-fu[1]*gv[0] + gu[0]*fv[1]-gu[1]*fv[0];
}

template<typename T>
inline void crossLong(T z[3], const T *fu, const T *fv, const T *gu, const T *gv, int n) {
    z[0] = fu[1*n]*gv[2*n]-fu[2*n]*gv[1*n] + gu[1*n]*fv[2*n]-gu[2*n]*fv[1*n];
	z[1] = -fu[0*n]*gv[2*n]+fu[2*n]*gv[0*n] - gu[0*n]*fv[2*n]+gu[2*n]*fv[0*n];
	z[2] = fu[0*n]*gv[1*n]-fu[1*n]*gv[0*n] + gu[0*n]*fv[1*n]-gu[1*n]*fv[0*n];
}
...
template<typename T>
inline T dot(const T *x, const T *y, int d) {
    T s = 0;
    for (int i=0; i<d; ++i) {
        s += x[i]*y[i];
    }
